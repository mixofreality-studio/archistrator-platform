...
// The ops.gen.ts diff must stay inside comments: the six generator-naming lines.
func TestOpsDiffIsCommentsOnly(t *testing.T) {
	pinned, err := os.ReadFile(filepath.Join(reference, "ops.gen.ts.diff"))
	if err != nil {
		t.Fatal(err)
	}
	var changed int
	for _, line := range strings.Split(strings.TrimSpace(string(pinned)), "\n") {
		if strings.HasPrefix(line, "@@") {
			continue
		}
		body := strings.TrimSpace(line[1:])
		if !strings.HasPrefix(body, "//") && !strings.HasPrefix(body, "*") {
			t.Errorf("ops.gen.ts differs from archistrator's in code, not a comment: %q", line)
		}
		changed++
	}
	if changed != 12 {
		t.Errorf("want 6 replaced comment lines (12 diff lines), got %d diff lines", changed)
	}
}
...
func reg(srv *mcp.Server) {
	...
	other.AddTool(srv, &mcp.Tool{Name: "notMCP"}, nil)
