// Package projectmodel parses a schema-first component contract document —
// the committed contract.schema.json that the archistrator codegen pipeline
// emits for every Method component. The document is a self-contained JSON
// Schema doc whose top-level $defs carry every I/O type and whose "interface"
// key carries the RPC surface descriptor (the part JSON Schema cannot express
// on its own).
//
// This module REPLACES the per-generator embedded copies of this parser
// (previously duplicated identically in framework-go-http-generator/contract
// and framework-go-mcp-generator/contract): it is the single shared owner.
// It is deliberately dependency-free (encoding/json only) so it builds
// standalone from its own go.mod.
