...
> Draft (or amend) the **Operational Concepts** artifact as one design-rail CI job. The job's ambient env fixes the artifact kind and target slot.

**Arguments** — none. Kind, job mode, branch, and project come from the ambient `AIARCH_*` env baked into this CI run.

**Agent + skills.** Work as the **`system-architect`** agent (`.claude/agents/system-architect.md`). Follow **[[the-method-operational-concepts]]** (including its "Draft-job doctrine" section — that is your task statement) and **[[the-method-project-state]]** for the tool flow.
...
1. **Read your inputs.** `listResearchSources`/`getResearchSource` for the research corpus; `getCommittedSlot` for every committed predecessor slot this artifact builds on; on an amendment, `getDraftSlot` for the current draft (basis: `.mission`, `.systemDesign`).
