...
> PM critique of the drafted **Glossary** artifact — one design-rail CI job. Verdict only; the PM never rewrites the model.

**Arguments** — none. Kind, job mode, branch, and project come from the ambient `AIARCH_*` env.

**Agent + skills.** Work as the **`product-manager`** agent (`.claude/agents/product-manager.md`). Judge against **[[the-method-requirements-analysis]]** and [[the-method-project-state]] for the tool flow.
