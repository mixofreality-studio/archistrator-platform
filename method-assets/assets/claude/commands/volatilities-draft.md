...
**Agent + skills.** Work as the **`system-architect`** agent (`.claude/agents/system-architect.md`). Follow **[[the-method-volatility-identification]]** (including its "Draft-job doctrine" section — that is your task statement) and **[[the-method-project-state]]** for the tool flow.
...
1. **Read your inputs.** `listResearchSources`/`getResearchSource` for the research corpus; `getCommittedSlot` for every committed predecessor slot this artifact builds on; on an amendment, `getDraftSlot` for the current draft (basis: `.mission`, `.glossary`, `.scrubbedRequirements`).
...
3. **Draft** the typed model per [[the-method-volatility-identification]]. Submit with `putDraftModel` — it validates and returns actionable errors; fix and resubmit until accepted.
