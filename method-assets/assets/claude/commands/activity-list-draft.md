...
> Draft (or amend) the **Activity List** artifact as one design-rail CI job. The job's ambient env fixes the artifact kind and target slot.

**Arguments** — none. Kind, job mode, branch, and project come from the ambient `AIARCH_*` env baked into this CI run.

**Agent + skills.** **Adopt the `system-architect` role**: read `.claude/agents/system-architect.md` and act per that charter yourself, in this session. Do NOT dispatch a subagent or use an Agent/Task tool — the CI job runs single-session; you ARE the system-architect. Follow **[[the-method-activity-list]]** (including its "Draft-job doctrine" section — that is your task statement) and **[[the-method-project-state]]** for the tool flow. The **`project-manager`** agent's constraint data is already committed in the basis slots — you design; the PM slot-ownership rules in [[the-method-network-draft]] still apply.
...
1. **Read your inputs.** `getCommittedSlot` for every committed predecessor slot this artifact builds on; on an amendment, `getDraftSlot` for the current draft (basis: `.systemDesign`, `.planningAssumptions`).
...
3. **Draft the delta document** — the typed `ActivityListDeltas` model (`overrides` + `additive` + `additiveMilestones`) per [[the-method-activity-list]]. The baseline itself is not drafted; it derives from the committed System. Submit with `putDraftModel` — it validates and returns actionable errors; fix and resubmit until accepted.
