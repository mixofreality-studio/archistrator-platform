...
> Draft (or amend) the **Core Use Cases** artifact as one design-rail CI job. The job's ambient env fixes the artifact kind and target slot.

**Arguments** — none. Kind, job mode, branch, and project come from the ambient `AIARCH_*` env baked into this CI run.

**Agent + skills.** **Adopt the `system-architect` role**: read `.claude/agents/system-architect.md` and act per that charter yourself, in this session. Do NOT dispatch a subagent or use an Agent/Task tool — the CI job runs single-session; you ARE the system-architect. Follow **[[the-method-core-use-cases]]** (including its "Draft-job doctrine" section — that is your task statement) and **[[the-method-project-state]]** for the tool flow.
...
3. **Draft** the typed model per [[the-method-core-use-cases]]. Every nonCore decision carries a `rejectionReason`; symmetrically, every CORE decision carries an `essenceRationale` — the essence-of-the-business argument (shared by all customers, nearly immutable, differentiating; an abstraction, not a feature). A core decision without one is flagged `DH-UC-ESSENCE-MISSING` (Warning). Submit with `putDraftModel` — it validates and returns actionable errors; fix and resubmit until accepted.
