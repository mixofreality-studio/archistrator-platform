...
**Agent + skills.** Work as the **`system-architect`** agent (`.claude/agents/system-architect.md`). Follow **[[the-method-system-design-standard-check]]** (including its "Draft-job doctrine" section — that is your task statement) and **[[the-method-project-state]]** for the tool flow.
...
1. **Read your inputs.** `listResearchSources`/`getResearchSource` for the research corpus; `getCommittedSlot` for every committed predecessor slot this artifact builds on; on an amendment, `getDraftSlot` for the current draft (basis: all Phase-1 slots).
...
3. **Draft** the typed model per [[the-method-system-design-standard-check]]. Submit with `putDraftModel` — it validates and returns actionable errors; fix and resubmit until accepted.
