...
|---|---|---|---|
| **Test engineer** | A full engineer who writes code to *break* the system. Not a tester. *"Every software project should have a test engineer."* | System Test Plan (`N-STP`; early, high-float); a perf rig only when a justified additive calls for one | `test-engineer` |
| **Software tester** | *Runs* system testing + regression; files defects. Wants **1:1–2:1** tester:developer ratio. | System Testing (`N-IT`; terminal) | `software-tester` |
...
Plus: **developers** write their own STP + unit tests. In Löwy's projects the
...
system and regression test harness**, so neither is an activity. If quality is
high enough, developers are *not needed during system testing* (ch11).
...
Testing contributes exactly **two** activities to the network — Table 11-1's
#4 (Test Plan) and #21 (System Testing):

| Activity | Position | Float | Owner | project.json target |
|---|---|---|---|---|
| System Test Plan (`N-STP`) | early (depends on M0) | high | test-engineer | `.testingState.systemTestPlan` |
| System Testing (`N-IT`) | **terminal, on critical path** — depends on every activity with no other successor | 0 | software-tester | `.testingState.testRuns` + `.testingState.defects` |

**Not activities:**
- **System + regression test harness** (Table 11-1 #5) — generated by the
  platform.
...
"consumes 77% of [its] float… very risky" (ch11). System Testing is the
...
- The **system** gets the System Test Plan and System Testing as activities;
  the System Test Harness and Regression Harness are platform-generated.
...
- The test plan and system testing = **direct cost**. The QA engineer, like
  every phase-spanning role, = **indirect cost**; ongoing regression + daily
  build/smoke run on platform-generated harness and build automation, also
  **indirect cost**.
