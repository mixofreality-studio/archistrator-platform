...
| 4 | CoreUseCases | `*projectstate.CoreUseCases` (`models_phase1.go`) | `decisions[]`: useCase + rejectionReason ("" when core); activity nodes carry `linkedComp` (component that performs each step, back-populated at architecture) |
| 5 | System | `*projectstate.System` (`system.go`) | `components[]`, `relationships[]`, `dynamicViews[]` — the layered architecture (Grammar A). Component ids here are kebab-case (e.g. `billing-engine`) — note this differs from the camelCase keys used in `.serviceContracts` (e.g. `billingEngine`). Also hosts `waivers[]` (relocated §2d + §naming standard-check waivers) and `attestations[]` (Prime/D1–D4/§3a–d semantic attestations). |
