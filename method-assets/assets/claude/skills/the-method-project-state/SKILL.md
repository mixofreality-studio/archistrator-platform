...
| 2 | ScrubbedRequirements | `*projectstate.ScrubbedRequirements` (`models_phase1.go`) | `items[]`: `id` (contiguous `B-NN`, no gaps) / `behavior` / `statedAs` (raw asks scrubbed from) / `volatilityHint` (step-4 hand-off). Customer label "Required Behaviors" (wire kind kept). |
| 3 | Volatilities | `*projectstate.Volatilities` (`models_phase1.go`) | `items[]`: name/rationale/axis/`traces` (→ Required-Behavior `B-NN` ids) + `rejected[]` ({name/reason/class}); also hosts `waivers[]` (relocated §2h standard-check waiver) |
| 4 | CoreUseCases | `*projectstate.CoreUseCases` (`models_phase1.go`) | `decisions[]`: useCase + rejectionReason ("" when core); every useCase carries a typed `activity` graph (nodes + guarded edges) whose action/event nodes are realized node-by-node by slot 5's dynamic-view steps. Nodes carry `roleName`/`linkedActorId` (swim lanes) and, on `decision`/`switch` only, an optional `decidedBy` naming a component or one of that use case's actors — **a component id living in the requirements slot, so a component rename must retarget it in the same amendment.** |
| 5 | System | `*projectstate.System` (`system.go`) | `components[]`, `relationships[]`, `dynamicViews[]` — the layered architecture (Grammar A). Each dynamic view is a step-keyed REALIZATION: `steps[]` of `{activityNodeId, calls[]}` where each call is `{from, to, mode, label, alt?}` and endpoints are component ids or the owning use case's actor ids (participants and sequence are derived, never stored). Component ids here are kebab-case (e.g. `billing-engine`) — note this differs from the camelCase keys used in `.serviceContracts` (e.g. `billingEngine`). Also hosts `waivers[]` (relocated §2d + §naming standard-check waivers) and `attestations[]` (Prime/D1–D4/§3a–d semantic attestations). |
| 6 | DeploymentOperationsModel | `*projectstate.DeploymentOperationsModel` (`models_phase1.go`) | per-project knobs (`deploymentScenario`/`constructionVenue`/`reviewPolicyRef`/`scalingPolicy`/`infraBuildingBlocks`) + `trustSummaries` + `deploymentModel` (topology view). Doctrine referenced from `platform-runtime-doctrine`, not stored. Customer label "Deployment & Operations Model" (wire kind `operationalConcepts` kept). |
| 7 | StandardCheck | `*projectstate.StandardCheck` (`models_phase1.go`) | The SYSTEM-design gate no longer commits here (Design Health teardown: live checks are render-on-read; waivers/attestations relocated onto slots 3 + 5). The `StandardCheck`/`CheckItem` $defs remain (reused by those relocated ledgers). The Phase-2 project-design standard check is a SEPARATE gate — see [[the-method-project-design-standard-check]]. |
