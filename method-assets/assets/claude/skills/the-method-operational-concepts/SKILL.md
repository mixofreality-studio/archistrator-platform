...
The deployment view is the deployment volatility made visible.

**START FROM [[webapp-deployment-prototype]].** Every web application built here shares one front door — a person on their device, the frontend surface they use, and an edge gateway that authenticates against the identity provider before forwarding to the application. That shape is generated from the platform baseline and enforced by `DEP-EDGE-GATEWAY`. Lift the fragment, rename its nodes for this project, add the project's own containers/infrastructure/external systems — but do NOT re-derive the front door.

**Relationships are half AUTHORED, half DERIVED.** The application edges (a Client calling a Manager, a ResourceAccess reading its Resource) are DERIVED from the committed `.systemDesign` relationships at read time — never author them here, or they will drift from the architecture. Author ONLY the edges whose endpoints are not System components at all: person → frontend, static-asset server → browser, frontend → gateway, gateway → identity provider, gateway → application container, and container → durable-execution substrate (the substrate is not an architecture edge, so nothing derives it).

**Every element carries a `key`** unique within its environment — nodes, container instances, infrastructure nodes, external systems and persons alike. A relationship addresses its endpoints by those keys.

Then apply these corrections (founder ruling §5):
...
| Deployment view starts from [[webapp-deployment-prototype]]; the gateway → identity-provider → application chain survives in every profile that HAS a gateway (`DEP-EDGE-GATEWAY`) | Lift the prototype rather than re-deriving the front door |
| Every environment instances ≥1 frontend surface — `spa` / `mobile` / `cli`, or an `agentHarness` element (`DEP-FRONTEND-PRESENT`) | Add the surface the customer actually uses |
| Every container instance, infrastructure node, external system and person has ≥1 edge, authored or derived (`DEP-EDGE-ISOLATED`) | Connect it or drop it — an element nothing reaches is decoration |
| Every element key is unique within its environment and every relationship endpoint resolves (`DEP-KEY-UNIQUE`, `DEP-EDGE-REF`) | Fix the keys |
...
Then populate the deployment-models view (`deploymentModel`) in C4-container shape, STARTING FROM [[webapp-deployment-prototype]] (the generated platform baseline: person → client device → browser/agent-harness → edge gateway → identity provider → application container). Do NOT re-derive that front door. The set of deployment environments is DERIVED from `deploymentScenario` and a test profile is ALWAYS present. Declare the top-level `containers` array — the deployable UNITS, not the components — each with a `key`, `name`, `technology`, `description`, and `components` listing the exact NAMES of the System components it packages. Every CODE component — every Client, Manager, Engine, and ResourceAccess, plus every Utility — MUST be packaged into EXACTLY ONE container. Resources are NOT container members — model each as an infrastructureNode (a self-describing name/technology/description) or, for a genuinely external third party, as a softwareSystemInstance. Each environment nests deploymentNodes whose containerInstances reference a declared container BY ITS `containerKey` and set an `instances` integer. CROSS-PROFILE INVARIANT: operating mode is configuration, not architecture — the set of deployed CONTAINERS MUST be IDENTICAL across environments (the underlying infrastructure MAY legitimately differ per profile). GIVE EVERY ELEMENT a `key` unique within its environment (nodes, containerInstances, infrastructureNodes, softwareSystemInstances, persons) — relationships address endpoints by key. Declare each container's `surface` (`spa` | `mobile` | `agentHarness` | `cli` | `service`) and each infrastructureNode/softwareSystemInstance's `role` (`gateway` | `identityProvider` | `database` | `objectStore` | `messaging` | `observability` | `agentHarness` | `sourceControl` | `paymentGateway` | `other`) — the rules find the front door by ROLE, never by matching the name "Envoy". AUTHOR the environment's `relationships` and `persons`: only the edges whose endpoints are NOT System components (person → frontend, asset server → browser, frontend → gateway, gateway → identity provider, gateway → application, container → durable-execution substrate). Do NOT author the application edges — they are DERIVED from the committed `.systemDesign` relationships at read time and would drift if restated. Every environment must instance at least one frontend surface, every element must carry ≥1 edge, and any environment with a `gateway` element must show the full frontend→gateway→identityProvider/application chain. Apply the §5 view corrections: OperatedRuntime is a workload node INSIDE the operated cluster (present only under `deployedOperated`), NOT an external service; the LOCAL profile shows the real local story (archistrator-server child process, embedded Temporal, on-disk `.aiarch` git, local `claude -p` runner) with an explicit "not present in local" annotation for the absent Postgres/Keycloak/GitHub-App/LLM-key; per-environment resources are env-accurate and scenario-gated (operate/bill stores appear only under `deployedOperated`). Reference containers by `containerKey` and System components by NAME.
...
- **A deployment view of boxes with no lines** — the one question a deployment view is asked is how anything reaches anything. Every element carries at least one edge.
- **Re-deriving the front door** — the gateway/identity-provider chain is generated in [[webapp-deployment-prototype]]; lift it. Re-deriving it is how the gateway ended up fronting nothing and the identity provider ended up filed under "external services".
- **Authoring the application edges** — a Client→Manager or ResourceAccess→Resource edge is DERIVED from the committed System model. Restating it here creates a second copy that drifts.
- **A deployment view with no frontend surface** — only back-end services means the model has omitted how the system is used.
- **A gateway in the LOCAL dev-boot profile** — a loopback bind has neither gateway nor identity provider; model the real local story instead of pretending.
